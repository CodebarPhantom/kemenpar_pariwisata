...
class EmergencyReportController extends Controller
{
    ...
    {
        if (!Laratrust::isAbleTo('view-emergency-report')) return abort(404);
        ...
        return DataTables::of($emergencyReports) 
            ...
            ->editColumn('status',function($emergencyReport){
                if($emergencyReport->status == 1){
                    $color = 'danger'; $status = 'New';
                }elseif($emergencyReport->status == 2){
                    $color = 'warning'; $status = 'In Handling';
                }elseif($emergencyReport->status == 3){
                    $color = 'success'; $status = 'Done';
                }
                return '<span class="badge bg-'.$color.' align-middle">'.Lang::get($status).'</span>';
            })  
            ->editColumn('date_report',function($emergencyReport){                
                return $emergencyReport->created_at ? with(new Carbon($emergencyReport->created_at))->translatedFormat('D, d-m-Y H:i') : '-';  
            }) 
            ...
            ->editColumn('action',function($emergencyReport){
                $show =  '<a href="'.route('report-emergency.show',$emergencyReport->id).'" class="btn btn-info btn-flat btn-xs align-middle" title="'.Lang::get('Show').'"><i class="fa fa-eye fa-sm"></i></a>';
                $edit =  '<a href="'.route('report-emergency.edit',$emergencyReport->id).'" class="btn btn-danger btn-flat btn-xs align-middle" title="'.Lang::get('Respond').'"><i class="fa fa-exclamation-triangle fa-sm"></i></a>';
                
                
                return $show.($emergencyReport->status == 1 || $emergencyReport->status == 2  && Laratrust::isAbleTo('view-emergency-report') ? $edit :'');
            })
    ...
    {
        if (!Laratrust::isAbleTo('create-emergency-report')) return abort(404);
    ...
    public function show($id)
    {
        ...
        $emergencyReport = EmergencyReport::select('emergency_reports.id','us.name as user_name','ti.name as tourism_name','emergency_reports.title','emergency_reports.status','emergency_reports.created_at','emergency_reports.description')
        ->leftJoin('users as us','us.id','=','emergency_reports.user_id')
        ->leftJoin('tourism_infos as ti','ti.id','=','emergency_reports.tourism_info_id')
