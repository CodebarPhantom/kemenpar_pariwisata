...
class TicketPromotionController extends Controller
{
    ...
    {
        if (!Laratrust::isAbleTo('view-ticket-promotion')) return abort(404);
    ...
    public function data()
    {
        ...
        $ticketPromotions = TicketPromotion::select('ticket_promotions.id','ticket_promotions.tourism_info_id','ticket_promotions.name','ticket_promotions.start_date','ticket_promotions.end_date','ticket_promotions.disc_percentage','ticket_promotions.status','ti.name as tourism_name')
                ->leftJoin('tourism_infos as ti','ti.id','=','ticket_promotions.tourism_info_id')->orderBy('ticket_promotions.end_date','DESC');
        return DataTables::of($ticketPromotions) 
            ...
            ->editColumn('start_date',function($ticketPromotion){
                return $ticketPromotion->start_date ? with(new Carbon($ticketPromotion->start_date))->translatedFormat('D, d-m-Y H:i') : '-';           

            })
            ->editColumn('end_date',function($ticketPromotion){
                return $ticketPromotion->end_date ? with(new Carbon($ticketPromotion->end_date))->translatedFormat('D, d-m-Y H:i') : '-';    
            })
            ...
            ->editColumn('action',function($ticketPromotion){
                $show =  '<a href="'.route('ticket-promotion.show',$ticketPromotion->id).'" class="btn btn-info btn-flat btn-xs" title="'.Lang::get('Show').'"><i class="fa fa-eye fa-sm"></i></a>';
                $edit =  '<a href="'.route('ticket-promotion.edit',$ticketPromotion->id).'" class="btn btn-danger btn-flat btn-xs" title="'.Lang::get('Edit').'"><i class="fa fa-pencil-alt fa-sm"></i></a>';
                 return $show.$edit;
            })
    ...
    {
        $ticketPromotion = TicketPromotion::select('ticket_promotions.id','ticket_promotions.tourism_info_id','ticket_promotions.name','ticket_promotions.start_date','ticket_promotions.end_date','ticket_promotions.disc_percentage','ticket_promotions.status','ti.name as tourism_name')
                            ->leftJoin('tourism_infos as ti','ti.id','=','ticket_promotions.tourism_info_id')->where('ticket_promotions.id',$id)->first();
    ...
    {
        $ticketPromotion = TicketPromotion::select('ticket_promotions.id','ticket_promotions.tourism_info_id','ticket_promotions.name','ticket_promotions.start_date','ticket_promotions.end_date','ticket_promotions.disc_percentage','ticket_promotions.status','ti.name as tourism_name','ti.code as tourism_code','ti.price as tourism_price')
                            ->leftJoin('tourism_infos as ti','ti.id','=','ticket_promotions.tourism_info_id')->where('ticket_promotions.id',$id)->first();
